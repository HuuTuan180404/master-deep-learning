...
class TNT(nn.Module):
    ...
    def forward_features(
        self, x, swap_index, moex_norm, moex_epsilon, moex_layer, moex_positive_only
    ):
        attn_weights = []
        B = x.shape[0]
        ...
        if swap_index is not None and moex_layer == "stem":
            patch_embed = moex(
                patch_embed, swap_index, moex_norm, moex_epsilon, moex_positive_only
            )
    ...
    def forward(
        ...
        x, attn_weights = self.forward_features(
            x, swap_index, moex_norm, moex_epsilon, moex_layer, moex_positive_only
        )
        x = self.head(x)
        if vis:
